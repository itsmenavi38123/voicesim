...
describe("Schedule Utilities", () => {
  ...
  describe("generateCronExpression", () => {
    it.concurrent(
      "should generate correct cron expressions for different schedule types",
      () => {
        ...
        expect(generateCronExpression("minutes", scheduleValues)).toBe(
          "*/15 * * * *"
        );

        // Hourly (at minute 45)
        expect(generateCronExpression("hourly", scheduleValues)).toBe(
          "45 * * * *"
        );

        // Daily (at 10:15)
        expect(generateCronExpression("daily", scheduleValues)).toBe(
          "15 10 * * *"
        );

        // Weekly (Monday at 12:00)
        expect(generateCronExpression("weekly", scheduleValues)).toBe(
          "0 12 * * 1"
        );

        // Monthly (15th at 14:30)
        expect(generateCronExpression("monthly", scheduleValues)).toBe(
          "30 14 15 * *"
        );
      }
    );
    ...
    it.concurrent("should handle custom cron expressions", () => {
      ...
      expect(generateCronExpression("minutes", standardScheduleValues)).toBe(
        "*/15 * * * *"
      );
    });
    ...
    it.concurrent("should throw for invalid schedule types", () => {
      ...
      expect(() =>
        generateCronExpression("invalid-type", scheduleValues)
      ).toThrow();
    });
  });
  ...
  describe("parseCronToHumanReadable", () => {
    it.concurrent("should parse common cron patterns", () => {
      ...
      expect(parseCronToHumanReadable("30 * * * *")).toBe(
      ...
      expect(parseCronToHumanReadable("0 9 * * 1")).toMatch(
        /Monday at 9:00 AM/
      );
      expect(parseCronToHumanReadable("30 14 15 * *")).toMatch(
  ...
  describe("createDateWithTimezone", () => {
    ...
    it.concurrent(
      "should correctly handle America/Los_Angeles (UTC-7 during DST)",
      () => {
    ...
    it.concurrent(
      "should correctly handle America/Los_Angeles (UTC-8 outside DST)",
      () => {
    ...
    it.concurrent(
      "should correctly handle America/New_York (UTC-4 during DST)",
      () => {
    ...
    it.concurrent(
      "should correctly handle America/New_York (UTC-5 outside DST)",
      () => {
    ...
    it.concurrent(
      "should correctly handle Europe/London (UTC+1 during DST)",
      () => {
    ...
    it.concurrent(
      "should correctly handle Europe/London (UTC+0 outside DST)",
      () => {
    ...
    it.concurrent(
      "should handle time crossing midnight due to timezone offset",
      () => {
